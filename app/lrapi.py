...
def input_graphs():
    ...
    df = pd.DataFrame({
        ...
        'mse': [
            np.mean((cross_val_score(reg, XScaled[feature_names], y, cv=cv, scoring='neg_mean_squared_error')) * -1)],
...
def optimal_lasso():
    ...
    df = pd.DataFrame({
        ...
        'optimalMSE': [np.mean((cross_val_score(Lasso(alpha=lasso_alpha.alpha_), XScaled, y, cv=cv,
